...
SELECT product_id,product_name,units_in_stock*unit_price as inventory_value FROM products;
...
SELECT LENGTH(product_name) lenght FROM products;

--Obtener una consulta de la tabla de productos que muestre el nombre en minúscula
SELECT product_id,LOWER(product_name),unit_price FROM products;
...
SELECT customer_id,company_name,country FROM customers WHERE country LIKE 'Spain';
...
SELECT customer_id,company_name,city,country FROM customers ORDER BY country,city ASC;
...
SELECT * FROM orders WHERE customer_id LIKE 'ALFKI' ORDER BY order_date DESC;

--Obtener los datos del detalle de ordenes, ordenados ascendentemente por precio cuyo producto sea 1, 5 o 20
SELECT * FROM order_details WHERE quantity IN(1,5,20) ORDER BY unit_price ASC;
...
SELECT MAX(unit_price),MIN(unit_price),AVG(unit_price) FROM products;
...
SELECT COUNT(*) FROM orders WHERE employee_id IN (1,3,5,6) GROUP BY employee_id;
...
SELECT ship_city,SUM(freight) AS suma_envio FROM orders WHERE ship_city IN ('Madrid','Sevilla','Barcelona','Lisboa','London') GROUP BY ship_city;

--obtener el precio promedio de los productos por categoria sin contar con los productos descontinuados (Discontinued)
SELECT AVG(unit_price) FROM products WHERE discontinued = 0 GROUP BY category_id;
...
SELECT COUNT(units_in_stock),category_id FROM products WHERE unit_price BETWEEN 10 AND 60 AND units_in_stock >12 GROUP BY category_id ;
...
SELECT SUM(units_in_stock) AS existencias FROM products WHERE supplier_id IN(17,19,16) GROUP BY category_id HAVING SUM(units_in_stock) < 100 ORDER BY existencias;
